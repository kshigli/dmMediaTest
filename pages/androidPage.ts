...
export class androidPage {
    ...
    async openGalleryView(){        
        await $('//android.widget.TextView[@text="News"]').click();
        await new Promise(f => setTimeout(f, 2000));
        await $('//androidx.recyclerview.widget.RecyclerView/android.widget.LinearLayout').click();
        await new Promise(f => setTimeout(f, 2000));
        await $('android=new UiScrollable(new UiSelector()).setAsHorizontalList().scrollTextIntoView("Titan victim Paul-Henri Nargeolet")');
        await new Promise(f => setTimeout(f, 2000));
        await $('//android.widget.Image[@text="Titan victim Paul-Henri Nargeolet "]').click();
        await new Promise(f => setTimeout(f, 2000)); 
        await $('android=new UiScrollable(new UiSelector()).setAsHorizontalList().scrollTextIntoView("Historic dive:")');        
